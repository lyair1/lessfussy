...
import { ChevronLeft, ChevronRight, Milk, Moon, Baby, Droplets, Pill, Thermometer, Activity, Ruler, Apple, Toilet, Trash2 } from "lucide-react";
...
export default function HistoryPage() {
  ...
  useEffect(() => {
    async function fetchEntries() {
      ...
      setLoading(true);
      try {
        const data = await getTimelineEntries(selectedBaby.id, date);
  ...
  if (!selectedBaby) {
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <p className="text-muted-foreground">Please select a baby to view history</p>
      </div>
    );
  }
