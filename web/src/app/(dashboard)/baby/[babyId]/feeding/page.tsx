...
export default function FeedingPage({
  ...
  const [activeTab, setActiveTab] = useState<FeedingTab>(
    editEntry?.type === "bottle" ? "bottle" : "nursing"
  );
  ...
  const [leftDuration, setLeftDuration] = useState(
    editEntry?.leftDuration || 0
  );
  const [rightDuration, setRightDuration] = useState(
    editEntry?.rightDuration || 0
  );
  const [pausedDuration, setPausedDuration] = useState(
    editEntry?.pausedDuration || 0
  );
  const [isTimerRunning, setIsTimerRunning] = useState(false);
  const [nursingStartTime, setNursingStartTime] = useState<Date | null>(
    editEntry?.startTime ? new Date(editEntry.startTime) : null
  );

  // Bottle state
  const [bottleContent, setBottleContent] = useState<BottleContent>(
    editEntry?.bottleContent || "breast_milk"
  );
  const [amountUnit, setAmountUnit] = useState<"oz" | "ml">(
    editEntry?.amountUnit || "oz"
  );
  const [amount, setAmount] = useState(editEntry?.amount || 4);
  const [bottleStartTime, setBottleStartTime] = useState(
    editEntry?.startTime ? new Date(editEntry.startTime) : new Date()
  );
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteFeeding(editEntry.id, babyId);
      ...
      async () => {
        ...
        try {
          if (editEntry) {
            ...
            await updateFeeding(editEntry.id, babyId, {
              startTime,
              endTime,
              leftDuration,
              rightDuration,
  ...
  return (
    <TrackingContainer>
      <TrackingHeader title={editEntry ? "Edit feeding" : "Add feeding"} />
