...
export default function PumpingPage({
  ...
  const [amountMode, setAmountMode] = useState<AmountMode>(
    editEntry?.leftAmount || editEntry?.rightAmount ? "left_right" : "total"
  );
  const [amountUnit, setAmountUnit] = useState<"oz" | "ml">(
    editEntry?.amountUnit || "oz"
  );
  const [leftAmount, setLeftAmount] = useState(editEntry?.leftAmount || 3);
  const [rightAmount, setRightAmount] = useState(editEntry?.rightAmount || 3);
  const [totalAmount, setTotalAmount] = useState(
    editEntry?.totalAmount || (editEntry?.leftAmount && editEntry?.rightAmount ?
      editEntry.leftAmount + editEntry.rightAmount : 6)
  );
  const [notes, setNotes] = useState(editEntry?.notes || "");
  const [saving, setSaving] = useState(false);
  const [loadingSession, setLoadingSession] = useState(!editEntry);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editEntry) {
        ...
        await updatePumping(editEntry.id, babyId, {
          startTime,
          endTime: editEntry.endTime ? new Date(editEntry.endTime) : new Date(),
          leftAmount: amountMode === "left_right" ? leftAmount : undefined,
          rightAmount: amountMode === "left_right" ? rightAmount : undefined,
          totalAmount:
            amountMode === "total" ? totalAmount : leftAmount + rightAmount,
  ...
  const handleDelete = async () => {
    ...
    try {
      await deletePumping(editEntry.id, babyId);
  ...
  return (
    <TrackingContainer>
      <TrackingHeader title={editEntry ? "Edit pumping" : "Add pumping"} />
