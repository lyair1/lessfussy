...
export default function SleepPage({
  ...
  const [elapsedSeconds, setElapsedSeconds] = useState(
    editEntry?.startTime && editEntry?.endTime ?
      Math.floor((new Date(editEntry.endTime).getTime() - new Date(editEntry.startTime).getTime()) / 1000) : 0
  );
  const [activeSleepId, setActiveSleepId] = useState<string | null>(
    editEntry?.id || null
  );

  const [detailsOpen, setDetailsOpen] = useState(editEntry ? true : false);
  const [startMood, setStartMood] = useState<Mood | null>(
    editEntry?.startMood || null
  );
  const [endMood, setEndMood] = useState<Mood | null>(
    editEntry?.endMood || null
  );
  const [fallAsleepTime, setFallAsleepTime] = useState<FallAsleepTime | null>(
    editEntry?.fallAsleepTime || null
  );
  const [sleepMethod, setSleepMethod] = useState<SleepMethod | null>(
    editEntry?.sleepMethod || null
  );
  const [wokeUpChild, setWokeUpChild] = useState(editEntry?.wokeUpChild || false);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteSleepLog(editEntry.id, babyId);
  ...
  return (
    <TrackingContainer>
      <TrackingHeader title={editEntry ? "Edit sleep" : "Add sleep"} />
      ...
      <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
        <DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            >
              {editEntry ? (saving ? "Saving..." : "Save") : "Done"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
